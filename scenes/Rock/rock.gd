...
func start(_position, _velocity, _size):
	...
	var circleCollisionShape = CircleShape2D.new()
	circleCollisionShape.radius = radius
	$CollisionShape2D.shape = circleCollisionShape
...
func _integrate_forces(physics_state):
	...
	xform.origin.y = wrapf(xform.origin.x, 0 - radius, screensize.y + radius)
